...
function findFirstSingleChar(str) {
  const arr = str.split('');
  return arr.find((el) => str.indexOf(el) === str.lastIndexOf(el));
}
...
function isCreditCardNumber(ccn) {
  let sum = 0;
  const arr = Array.from(ccn.toString(), (el) => Number(el)).reverse();
...
function getDigitalRoot(num) {
  const arr = Array.from(num.toString(), (el) => Number(el));
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (openedBrackets.includes(curr)) {
      stack.push(curr);
    } else {
      if (stack.length === 0) {
        return false;
      ...
      const topElemStack = stack[stack.length - 1];

      if (pairsBrackets[curr] === topElemStack) {
        stack.pop();
      } else {
        return false;
      }
...
function getCommonDirectoryPath(pathes) {
  const dirArrays = pathes.map((path) => path.split('/'));

  const minLength = Math.min(...dirArrays.map((arr) => arr.length));

  let commonDir = '';
  for (let i = 0; i < minLength; i += 1) {
    const dir = dirArrays[0][i];

    if (dirArrays.every((arr) => arr[i] === dir)) {
      commonDir += `${dir}/`;
    } else {
      break;
    }
